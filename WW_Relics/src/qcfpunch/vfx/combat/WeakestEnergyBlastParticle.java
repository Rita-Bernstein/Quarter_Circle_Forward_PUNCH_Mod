...
//Base code copied from HemokinesisParticle
public class WeakestEnergyBlastParticle extends AbstractGameEffect
{
	...
	public WeakestEnergyBlastParticle(float sX, float sY, float tX,
  			float tY, boolean facingLeft) {
		
		this.crs = new CatmullRomSpline();
		this.controlPoints = new ArrayList<Vector2>();
		this.points = new Vector2[1];
	    ...
	    this.currentSpeed = 250.0F * Settings.scale;
	...
	private void updateMovement() {
	    Vector2 tmp = new Vector2(this.pos.x - this.target.x,
	    ...
	    if (this.target.dst(this.pos) < DST_THRESHOLD) {
	    	for (int i = 0; i < 5; i++) {
	    		if (this.facingLeft) {
	    			AbstractDungeon.effectsQueue.add(
	    					new DamageImpactLineEffect(
	    							this.target.x + DST_THRESHOLD, this.target.y));
		        } else {
		        	AbstractDungeon.effectsQueue.add(
		        			new DamageImpactLineEffect(
		        					this.target.x - DST_THRESHOLD, this.target.y));
	    	...
	    	CardCrawlGame.sound.playAV("BLUNT_HEAVY",
	    			MathUtils.random(0.6F, 0.9F), 0.5F);
